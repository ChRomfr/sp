	...
	public function add($data){
		...
		if( $data['parent_id'] == 0){
			...
			if( is_null($Result['lft']) && is_null($Result['rght']) ){
				$Query = 'INSERT INTO '. $this->Table .' (`name`, lft, rght, level, parent_id, description, image) VALUES ("'. $data['name'] .'", 1,2,0,0,"'.  $data['description'].'","")';
				$this->Db->query($Query);
			}else{
				$this->Db->insert($this->Table, array('name' => $data['name'], 'lft' => $Result['rght']+1, 'rght' => $Result['rght']+2, 'parent_id' => 0, 'level' => 0, 'description' => $data['description']) );
			}
